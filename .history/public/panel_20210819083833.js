class Panel {
   ...
   passDataToFields(data) {
      ...
      // Изображения
      if(data.images) {
         const imageWrapperBlock = this.query('.productImagesDiv').classList.remove('db')
         imageWrapperBlock.classList.remove('db')
         const imagesBlock = this.query('#productImages')

         let imgNum = 0
         for(let image of data.images) {
            const divTag = `<div class='img_num_${imgNum}'>${image}</div>`
