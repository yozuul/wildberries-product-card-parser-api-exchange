...
class Panel {
   ...
   parseButton() {
      this.button.parseCard.onclick = async () => {
         ...
         // Получение данных
         // const parseData = await this.apiFetch(this.api.parseCard, inputs.cardProductURL.value)
         // console.log(this.api.getAdditionalCardData);
         // console.log(this.api.getAdditionalCardData, parseData);
         // const extraData = await this.apiFetch(this.api.getAdditionalCardData, parseData)
         // console.log(this.api.getAdditionalCardData, parseData);

         // console.log();
         // this.getAdditionalCardData(data)
         this.apiFetch2()
      }
   }
   ...
   async apiFetch2() {
      ...
      try {
         const response = await fetch(this.api.getAdditionalCardData, {
            method: 'POST',
            body: JSON.stringify({ data: newest }),
