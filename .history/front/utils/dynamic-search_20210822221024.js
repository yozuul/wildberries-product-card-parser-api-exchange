   ...
   async tryFoundTvend(foundedResult) {
      if(foundedResult) {
         ...
         if(tnved) {
            ...
            const foundedTvned = []
            for (const item in tvnedResult) {
               foundedTvned.push(`${tvnedResult[item].tnvedCode} ${tvnedResult[item].description}`)
            }
            foundedTvned.reverse().map((item) => {
               push(item).after.begin(select.searchProductTnved)
            })
         }
      }
   }
