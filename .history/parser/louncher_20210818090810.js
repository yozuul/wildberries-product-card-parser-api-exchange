...
class Louncher {
   ...
   async parseURL(url) {
      ...
      const queries = {
         brand: '[data-link="text{:productCard^brandName}"]',
      }

      let data = {}

      for(let query in queries) {
         data[query] = await page.evaluate((queries) => { // <-- use it as parameter of this function
            console.log(queries.brand); // <-- use it inside as you wish
         }, queries);
         // data[query] = await page.$eval(queries[query], (e) => e)
      }

      // const html = await page.$eval(query.brand, (e) => e)
