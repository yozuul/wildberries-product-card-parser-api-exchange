...
class Louncher {
   ...
   async parseCardURL(url) {
      ...
      const fields = {
         ...
         desc: await page.evaluate(() => document.querySelector('[property="og:description"]').content),

         brand: await page.evaluate(() => document.querySelector('.price-block__final-price').innerText),

         price: await page.evaluate(() => document.querySelector('[data-link="text{:productCard^brandName}"]').innerText),

         images: await page.evaluate(async () => await parseImages()),
      }
