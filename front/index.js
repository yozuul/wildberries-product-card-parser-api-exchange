...
import { button, input, element, textarea, FillFields, ChangedData, PrepareData } from './data-process/index'
...
class ParsePanel {
   ...
   saveToken() {
      button.saveToken.onclick = async () => {
         const response = await API.saveToken(textarea.token.value)
         if(response.save == 'ok') {
